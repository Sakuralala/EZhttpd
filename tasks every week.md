...
再后续：似乎找到了原因:accept需要不断直至没有新连接。不然的话由于ET模式，可能就没了；但是问题是，为什么呢？就算一次只accept一个，后续只要新连接来也应该能使得监听事件就绪啊？？难道是因为并发连接太多导致listen挂了？？(TODO:  )      
再再后续：找到了一篇博客分析了这种情况:使用ET的epoll监控监听描述符时，如果在新连接到来是只是调用accept一次而不是循环调用，假设来了好几个连接，那么这次accept只取出一个，剩下的就只能等在就绪队列中，然后等到对端关闭连接，而这边只是将其标记为只读，这样下一次再有新连接到来时取得是这个已经关闭的连接(accept默认取队列头部的),那么往这个连接发数据就会出错(经过log日志及netstat验证，的确会出现这种现象，即accept的连接端口号和netstat显示的不一致);这样一来就导致了后续的连接都出错了。   
